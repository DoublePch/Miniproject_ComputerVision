...
def robert(imgpa): #ประกาศชื่อ ฟังก์ชั่นชื่อว่า robert แล้วรับค่าพารมิเตอร์ imgpa
    ...
    img_gaussian = cv2.GaussianBlur(gray,(5,5),1) # ใช้ GaussianBlur เพื่อทำการเบลอภาพแล้วเก็บค่าที่ img_gaussian
    ...
    lines = cv2.HoughLinesP(imgrobert,1, np.pi/180, threshold, minLineLength=1,maxLineGap=2)
...
def sobel(imgpa):
    ...
    lines_list =[]
    lines = cv2.HoughLinesP(imgsobel,1, np.pi/180, threshold, minLineLength=5,maxLineGap=10)
...
def prewitt(imgpa):
    ...
    lines_list =[]
    lines = cv2.HoughLinesP(imgpree,1, np.pi/180, threshold, minLineLength=5,maxLineGap=10)
...
def canny(imgpa):
    ...
    img = cv2.imread(imgpa)
    gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    edgesCanny = cv2.Canny(gray,50,200,apertureSize=3)
    # print(type(img))
    # print(img.ndim)
    ...
    lines_list =[]
    lines = cv2.HoughLinesP(edgesCanny,1,np.pi/180, threshold, minLineLength=5,maxLineGap=10)
