...
orig = cv2.imread('cir1.jpg',0)

processed = cv2.cvtColor(orig,cv2.COLOR_GRAY2BGR)
storage = cv2.cv.CreateMat(orig.width, 1, cv2.CV_32FC3)


#use canny, as HoughCircles seems to prefer ring like circles to filled ones.
cv2.Canny(processed, processed, 5, 70, 3)
#smooth to reduce noise a bit more
cv2.Smooth(processed, processed, cv2.GAUSSIAN, 7, 7)

cv2.HoughCircles(processed, storage, cv2.HOUGH_GRADIENT, 2, 32.0, 30, 550)
